            ...
            <div class="row book-main-block">
                <div class="col-md-5" style="padding-right: 5%">
                    ...
                    <div class="book_read text-center default" style="margin-top: 10px">
                        ...
                        @if(Auth::check()&&Auth::user()->favorites->where('id', $artwork->id)->first()==null)
                            <a href="{{ route('addToFavorite', ['artwork_id'=>$artwork->id, 'user_id' => Auth::id()]) }}"
                               class="btn btn-outline-primary btn-lg my-2 my-sm-0 btn-sm btn-block" role="button" aria-pressed="true">
                                <span>Добавить в избранное</span>
                            </a>
                        @elseif(Auth::check()&&Auth::user()->favorites->where('id', $artwork->id)->first()!=null)
                            <a href="{{ route('deleteFromFavorites', ['artwork_id'=>$artwork->id, 'user_id' => Auth::id()]) }}"
                               class="btn btn-outline-danger btn-lg my-2 my-sm-0 btn-sm btn-block" role="button" aria-pressed="true">
                        ...
                        <div style="display: inline-block">
    @if(Auth::check())
            <a class="btn btn-outline-warning btn-block btn-sm" data-toggle="modal" data-target="#complaintDescriptionModal">
                <span>Пожаловаться</span>
            </a>
        <div class="modal fade" id="complaintDescriptionModal" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="buyModalLabe2">Жалоба на книгу {{ $artwork->title }}</h5>
                    ...
                    <div class="modal-body">
                            ...
                            <form class="form-inline" method="POST"
                                  action="{{ route('storeMessage', ['id'=>Auth::id()]) }}">
                                <div class="form-group">
                                    <textarea name="text" class="form-control" placeholder="Текст жалобы"></textarea>
                                </div>
                                ...
                                <button style="text-align: right" type="submit"
                                        class="btn btn-success btn-md text-right">Пожаловатся
                                </button>
